...
- `DB_HOST` tiene default `10.124.80.4` (IP privada de la VPC, usada por
  Cloud Run en producción). Para desarrollo local, sobreescribir en `.env`
  con la IP pública `34.70.133.119`.
- `DB_USER` y `DB_PASSWORD` NO tienen default: si no están en el entorno,
  las funciones de BD fallarán con un mensaje amigable (DatabaseError).
"""
...
class Settings:
    ...
    # Base de datos
    # DB_HOST: default a la IP privada de la VPC (producción, Cloud Run).
    # Para dev local, sobreescribir en `.env` con `DB_HOST=34.70.133.119`.
    ...
    # Credenciales SIN default — deben venir de env o secrets.
    DB_USER: str | None = _get_secret("DB_USER")
    DB_PASSWORD: str | None = _get_secret("DB_PASSWORD")
